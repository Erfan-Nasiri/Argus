...
local SECTION_CACHE_TTL = 300
local section_name_cache = {}
local last_cache_cleanup = os_time()

function M.clear_cache()
    section_name_cache = {}
    last_cache_cleanup = os_time()
end

function M.get_cache_stats()
    local count = 0
    for _ in pairs(section_name_cache) do
        count = count + 1
    end
    return {
        size = count,
...
function M.resolve_section_name(config, section_id, section_type, operation_data)
    ...
    local cached = section_name_cache[cache_key]
    if cached and (os_time() - cached.timestamp) < SECTION_CACHE_TTL then
        return cached.name
    end
    ...
    section_name_cache[cache_key] = { name = resolved_name, timestamp = os_time() }
