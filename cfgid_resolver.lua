...
-- Execute UCI command with comprehensive error handling (local helper to avoid forward refs)
local function execute_command(command)
    local full_command = "uci " .. command .. " 2>/dev/null"
    local success, handle = pcall(io_popen, full_command, "r")
    if not success or not handle then return nil end
...
local uci_interface = {
    ...
    get_section_data = function(config, cfgid)
        ...
        local output = execute_command("show " .. config .. "." .. cfgid)
        ...
        for line in string_gmatch(output, "[^\n]+") do
            local section_type = string_match(line, "^" .. config .. "%." .. cfgid .. "=(.+)$")
            if section_type then
                section_data.type = section_type
            else
                local option, value = string_match(line, "^" .. config .. "%." .. cfgid .. "%.([^=]+)=(.+)$")
                if option and value then
                    local clean_value = string_match(value, "^'(.*)'$") or value
                    section_data[option] = clean_value
                end
            end
        end

        return section_data.type and section_data or nil
    end,

    -- Batch get multiple sections (performance optimization)
    get_multiple_sections = function(config, cfgids)
        local results = {}
        local output = execute_command("show " .. config)
        if output then
            local section_data_map = {}
            for line in string_gmatch(output, "[^\n]+") do
                for _, cfgid in ipairs(cfgids) do
                    if string_find(line, config .. "." .. cfgid) then
                        if not section_data_map[cfgid] then
                            section_data_map[cfgid] = {}
                        end

                        local section_type = string_match(line, "^" .. config .. "%." .. cfgid .. "=(.+)$")
                        if section_type then
                            section_data_map[cfgid].type = section_type
                        else
                            local option, value = string_match(line, "^" .. config .. "%." .. cfgid .. "%.([^=]+)=(.+)$")
                            if option and value then
                                local clean_value = string_match(value, "^'(.*)'$") or value
                                section_data_map[cfgid][option] = clean_value
                            end
                        end
                    end
                end
            ...
            for _, cfgid in ipairs(cfgids) do
                results[cfgid] = section_data_map[cfgid]
            end
        end
    ...
    is_available = function()
        return execute_command("show system.@system[0]") ~= nil
    end
}

-- Allow `uci_interface.execute_command` to override the local execute_command
do
    local _original_execute = execute_command
    execute_command = function(cmd)
        if uci_interface and uci_interface.execute_command and uci_interface.execute_command ~= _original_execute then
            return uci_interface.execute_command(cmd)
        end
        return _original_execute(cmd)
    end
end
